...
def proportional_sampling(df: pd.DataFrame, group_column: str, total_samples: int):
    ...
    if total_samples > len(df):
        raise ValueError(f"total_samples ({total_samples}) cannot exceed the number of rows in the dataframe ({len(df)})")
    ...
    assert len(result) == total_samples, f"Result length ({len(result)}) does not match total_samples ({total_samples})"
    return result
