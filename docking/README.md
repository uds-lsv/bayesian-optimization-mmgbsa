...
2. Extract the molecules from the `similarity_scores.csv` file
    ```bash
...
4. Start docking
    ...
    ./start_docking
    ```
5. Once docking is finished run post-processing such as extracting the `.mol` blocks from the database.
    ```bash
